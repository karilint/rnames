...
def macrostrat_import(res_sn_raw):
    ...
    relations_MS.rename(columns={"Chrono_global": "Location_name_one"},inplace = True)
    relations_MS = pd.merge(relations_MS, dfx, left_on='Qualifier_two', right_on= 'Chrono_global', how='outer')
    relations_MS.rename(columns={"Chrono_global": "Location_name_two"},inplace = True)
    ...
    ###### MS structural names
    structured_names_MS = relations_MS[['Name_one', 'Qualifier_one', 'Location']]

    ################################
    #### output#####################
    #### structural names out out
    structured_names_MS = relations_MS[['Name_one', 'Qualifier_one', 'Location']]
    structured_names_MS = sn_MS.drop_duplicates()
    #### relations out
