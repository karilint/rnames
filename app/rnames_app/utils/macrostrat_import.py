...
def macrostrat_import(res_sn_raw):
    ...
    relations_MS.rename(columns={"Location": "Location_one"},inplace = True) # Location changed from xx Chrono_global
    relations_MS = pd.merge(relations_MS, dfx, left_on='Qualifier_two', right_on= 'Chrono_global', how='outer')
    relations_MS.rename(columns={"Location": "Location_two"},inplace = True)
    relations_MS['Name_one_remarks'] ='Autogenerated by Macrostrat importer.'
    relations_MS['Name_two_remarks'] ='Autogenerated by Macrostrat importer.'
    relations_MS = relations_MS.dropna(subset=['Name_one', 'Name_two', 'Qualifier_one', 'Qualifier_two'])
    relations_MS = relations_MS[(relations_MS['Name_one'] != '') & (relations_MS['Name_two'] != '')]

    relations_MS['Location_one'] = relations_MS['Location_one'].fillna('not given')
    relations_MS['Location_two'] = relations_MS['Location_two'].fillna('not given')

    ###### MS structural names
    structured_names_MS_1 = relations_MS[['Name_one', 'Qualifier_one', 'Location_one']]
    structured_names_MS_2 = relations_MS[['Name_two', 'Qualifier_two', 'Location_two']]
    structured_names_MS_1.rename(columns={'Name_one':'name', 'Qualifier_one':'qualifier_name',
                                 'Location_one': 'location'}, inplace=True)
    structured_names_MS_2.rename(columns={'Name_two':'name', 'Qualifier_two':'qualifier_name',
                                 'Location_two':'location'}, inplace=True)
    structured_names_MS = pd.concat([structured_names_MS_1, structured_names_MS_2])
    ################################
    #### output#####################
