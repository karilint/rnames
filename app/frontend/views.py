...
def index(request, pk=None, *args, **kwargs):
    ...
    if pk:
        ref = Reference.objects.filter(is_active=True).get(pk=pk)
        rels = list(Relation.objects.filter(is_active=True, reference_id=pk).values('id', 'name_one_id', 'name_two_id', 'belongs_to'))
